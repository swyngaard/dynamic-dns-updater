...
def get_subdomain_property(cpanel, property):
    params = cpanel['params']
    params['cpanel_jsonapi_func'] = 'fetchzone'
    
    response = requests.get(cpanel['url'], params, auth=(cpanel['user'], cpanel['pass']),  verify=False)
...
def edit_subdomain_address(cpanel, dns_line, dns_address):
    ...
    requests.get(cpanel['url'], params, auth=(cpanel['user'], cpanel['pass']),  verify=False)
...
def main():
    ...
    parser.add_argument("sub_domain", nargs='+', help="List of sub-domain names to be updated (excluding the qualified domain suffix). eg. hello.example.com would simply translate to hello")
